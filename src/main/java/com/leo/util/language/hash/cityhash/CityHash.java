package com.leo.util.language.hash.cityhash;

import com.leo.util.language.hash.Hashable;
import com.leo.util.language.hash.Number128;
...
public class CityHash implements Hashable {
  ...
  private Number128 hash128WithSeed(final byte[] byteArray, int start, final Number128 seed) {
    ...
    for (int tailDone = 0; tailDone < len; ) {
      ...
      z += w.getHiValue() + fetch64(byteArray ,pos + len - tailDone);
      w.setHiValue(w.getHiValue() + v.getLowValue());
      v = weakHashLen32WithSeeds(byteArray, pos + len - tailDone, v.getLowValue() + z, v.getHiValue());
      v.setLowValue(v.getLowValue() * K0);
    }
...
// in that case.
  ...
  private long loadUnaligned64(final byte[] byteArray,final int start) {
    long result = 0;
    OrderIter orderIter = new OrderIter(8,CityHash.IS_BIG_EDIAN);
  ...
  private int loadUnaligned32(final byte[] byteArray,final int start) {
    int result = 0;
    OrderIter orderIter = new OrderIter(4,CityHash.IS_BIG_EDIAN);
    while(orderIter.hasNext()){
      int next = orderIter.next();
  ...
  private long hash128to64(final Number128 number128) {
    // Murmur-inspired hashing.
    long a = (number128.getLowValue() ^number128.getHiValue()) * K_MUL;
    a ^= (a >>> 47);
    long b = (number128.getHiValue() ^ a) * K_MUL;
    b ^= (b >>> 47);
    b *= K_MUL;
    return b;
  }
